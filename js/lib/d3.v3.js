  ...
  d3.svg.brush = function() {
    ...
    function brush(g) {
      g.each(function() {
        ...
        }).attr("map_width", 6).attr("map_height", 6).style("visibility", "hidden");
        tz.style("display", brush.empty() ? "none" : null);
        tz.exit().remove();
        if (x) {
          e = d3_scaleRange(x);
          bg.attr("x", e[0]).attr("map_width", e[1] - e[0]);
          redrawX(g);
        }
        if (y) {
          e = d3_scaleRange(y);
          bg.attr("y", e[0]).attr("map_height", e[1] - e[0]);
          redrawY(g);
        }
        redraw(g);
      });
    ...
    function redrawX(g) {
      ...
      g.selectAll(".extent,.n>rect,.s>rect").attr("map_width", extent[1][0] - extent[0][0]);
    }
    function redrawY(g) {
      g.select(".extent").attr("y", extent[0][1]);
      g.selectAll(".extent,.e>rect,.w>rect").attr("map_height", extent[1][1] - extent[0][1]);
    }
    function brushstart() {
